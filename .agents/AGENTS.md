...
### 2. Devcontainer Networking & Integration Tests
- The devcontainer is configured with `--network host`, sharing the host's networking stack. 
- **Rule**: Running integration tests (`./build.sh --run-integration-tests [--debug]`) MUST be performed **only from within the devcontainer** (e.g., via `.devcontainer/run_in_docker.sh ./build.sh --run-integration-tests --debug`).
...
### 3. Unit & Integration Testing & Verification Commands
- **Full Unit Test Suite**: Before merging or finalizing changes, run the entire unit test suite:
  ```bash
  ./build.sh --debug --run-tests
  ```
- **Integration Test Suite**: Running integration tests must be performed **only from within the devcontainer**:
  ```bash
  .devcontainer/run_in_docker.sh ./build.sh --run-integration-tests --debug
  ```
- **Fast Iteration**: For targeted debugging or rapid iteration on index algorithms, build and run specific test suites (e.g., `indexes_test`):
  ```bash
  ninja -C .build-debug indexes_test && ./.build-debug/tests/indexes_test --gtest_brief=1
  ```
...
- Always compile and validate changes using strict `clang-tidy` thread-safety annotations from **within the devcontainer** (regardless of whether it was built in release or debug mode):
  ```bash
  .devcontainer/run_in_docker.sh clang-tidy -p .build-release/compile_commands.json \
...
- **Merge Commits**: Always include the `--signoff` (`-s`) flag when merging branches (`git merge --signoff <branch>`).
- **Post-Merge Formatting**: Whenever merge conflicts are resolved, run `.devcontainer/run_in_docker.sh ./ci/clang-format-changes.sh` before committing to prevent formatting regressions from merging divergent branches.
