...
| ------- | ---------- | ------------ | ------------ |
| 1.2.1   | `search_compatibility-ft_aggregate_load_as` | `AS` is treated as an ordinary field name, and a JSON path cannot be used as a `LOAD` field. | `FT.AGGREGATE` `LOAD` honors the `AS <alias>` rename clause and accepts a JSON path as the field to load; two renames to the same alias are rejected. |
