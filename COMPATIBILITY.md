...
| 1.3.0   | `compatibility-ft_aggregate_load_json_path` | A JSON path cannot be used as the field of an `FT.AGGREGATE` `LOAD` clause; the path is left unresolved and the load fails. | A JSON path is accepted as the field to load and is resolved to the attribute it identifies. |
