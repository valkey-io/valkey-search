...
- `FLAT:` This algorithm provides exact answers, but has runtime proportional to the number of indexed vectors and thus may not be appropriate for large data sets.
  ...
  - `TYPE [FLOAT32 | FLOAT16 | BFLOAT16 | FLOAT64]` (required): Element data type. `FLOAT16` and `BFLOAT16` halve the memory used per vector at some cost in precision; `FLOAT64` doubles it for higher precision; see [Supported Data Types](../topics/search-data-formats.md#supported-data-types).
