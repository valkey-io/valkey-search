...
The search module parses this string internally, splitting on commas (with whitespace skipped) and converting each element to the index's declared `TYPE`. For a `FLOAT16` or `BFLOAT16` index the text is therefore rounded to 2-byte precision on ingest, exactly as a binary blob would be.
