...
The search module sues the Valkey configuration mechanism. Thus each of the named configuration below can be set on the module load command OR via the `CONFIG SET` command.

Fields marked as "Immutable" can only be changed at startup

Fields marged as "Safe to modify" can safely be modified on a running system.

| Name                                          |  Units  | Immutable | Safe to modify | Description                                                                                                                       |
| :-------------------------------------------- | :-----: | :-------: | :------------: | :-------------------------------------------------------------------------------------------------------------------------------- |
| search.query-string-bytes                     | Number  |           |      Yes       | Controls the length of the query string for FT.SEARCH command                                                                     |
| search.hnsw-block-size                        | Number  |           |      Yes       | HNSW resize configuration parameter for vector index block allocation                                                             |
| search.reader-threads                         | Number  |           |      Yes       | Controls the reader thread pool size; dynamically resizes pool on modification                                                    |
...
| search.use-coordinator                        | Boolean |    Yes    |                | Controls whether this instance uses coordinator; can only be set at startup                                                       |
...
| search.info-developer-visible                 | Boolean |           |                | Controls visibility of DEV (developer) metrics in info search                                                                     |
