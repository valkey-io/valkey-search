...
class TestAggregateCompatibility(BaseCompatibilityTest):
    ...
    def test_alias_search(self, key_type, dialect):
        ...
        self.execute_command(["FT.INFO", alias])
...
class TestAliasCompatibility(BaseCompatibilityTest):
    ...
    def test_aliasupdate_search(self, key_type):
        """FT.ALIASUPDATE reassigns alias; queries reflect the new target."""
        alias = "alias_update_compat"
        self.execute_command(["FT.ALIASADD", alias, f"{key_type}_idx2"])
        self.execute_command(["FT.SEARCH", alias, "@price:[0 +inf]"])
        self.execute_command(["FT.ALIASUPDATE", alias, f"{key_type}_idx1"])
        self.execute_command(["FT.INFO", alias])
        self.execute_command(["FT.SEARCH", alias, "@price:[0 +inf]", "SORTBY", "price", "ASC"])
        self.execute_command(["FT.ALIASDEL", alias])

    def test_alias_after_index_dropped(self, key_type):
        """Dropping the target index makes the alias unusable."""
        idx = f"{key_type}_idx1"
        self.client.execute_command("FT.DROPINDEX", idx)
        # alias_search pointed to idx — should now error
        self.execute_command(["FT.INFO", "alias_search"])
        self.execute_command(["FT.SEARCH", "alias_search", "*"])
        # Restore state for subsequent tests
        if key_type == "hash":
            self.client.execute_command(
                "FT.CREATE", idx, "ON", "HASH", "PREFIX", "1", "adoc:",
                "SCHEMA", "price", "NUMERIC", "category", "TAG")
        else:
            self.client.execute_command(
                "FT.CREATE", idx, "ON", "JSON", "PREFIX", "1", "jdoc:",
                "SCHEMA", "$.price", "AS", "price", "NUMERIC", "$.category", "AS", "category", "TAG")
        self.client.execute_command("FT.ALIASADD", "alias_search", idx)
        self.client.execute_command("FT.ALIASADD", "alias_agg", idx)
