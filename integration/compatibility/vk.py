#!/usr/bin/env python3
"""
Simple script to test Valkey with search queries.
Assumes Valkey server is already running on localhost:6379.
"""
import redis
import time
...
DIALECT = 2
QUERY = 'quiet build (shark six ocean) onion'
search_cmd = ["FT.SEARCH", INDEX_NAME, QUERY, "SLOP", "9", "DIALECT", str(DIALECT)]
compat_mode = True
...
def main():
    # Connect to Valkey (assumes it's already running)
    print("Connecting to Valkey on localhost...")
    client = redis.Redis(host='localhost', port=7001, decode_responses=False)
