...
class FTInfoParser:
    ...
    def _parse_response(self, response: List[Any]) -> Dict[str, Any]:
        ...
        if not isinstance(response, list) or len(response) % 2 != 0:
            raise ValueError("FT.INFO response must be a list with even number of elements")
    ...
    def _parse_key_value_list(self, data: List[Any]) -> Union[Dict[str, Any], List[Any]]:
        ...
        for attr in attributes:
            if isinstance(attr, list):
                ...
                if isinstance(parsed_attr, dict):
                    ...
                    if 'index' in parsed_attr and isinstance(parsed_attr['index'], list):
                        ...
                        if isinstance(index_parsed, dict):
                            ...
                            if 'algorithm' in parsed_attr['index'] and isinstance(parsed_attr['index']['algorithm'], list):
                                algorithm_parsed = self._parse_key_value_list(parsed_attr['index']['algorithm'])
        ...
        if self.attributes:
            ...
            for i, attr in enumerate(self.attributes):
                ...
                if attr.get('type') == 'VECTOR' and 'index' in attr:
                    ...
                    lines.append(f"      Dimensions: {index_info.get('dimensions', 'Unknown')}")
                    lines.append(f"      Distance Metric: {index_info.get('distance_metric', 'Unknown')}")
