...
function run_pytest() {
  zap valkey-server
  LOG_INFO "Running: ${PYTHON_PATH} -m pytest ${FILTER_ARGS} -s --cache-clear -v ${ROOT_DIR}/integration/"
  ${PYTHON_PATH} -m pytest ${FILTER_ARGS} --full-trace -s --cache-clear -v ${ROOT_DIR}/integration/
  RUN_SUCCESS=$?
}
