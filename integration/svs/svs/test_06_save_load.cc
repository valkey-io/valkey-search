...
int main() {
  ...
  // Build, save.
  {
    ...
    st = idx->search(1, q.data(), 10, dists.data(), slabels.data(), nullptr,
                     nullptr);
    if (!st.ok()) {
      fail("search-pre-save", st.message());
      DVamana::destroy(idx);
      return 1;
    }
    pre_labels = slabels;
  ...
  std::vector<size_t> post_labels(10, SIZE_MAX);
  {
    ...
    if (!st.ok()) {
      fail("search-post-load", st.message());
  ...
  for (size_t i = 0; i < 10; ++i)
    if (pre_labels[i] == post_labels[i] && pre_labels[i] != SIZE_MAX) ++matches;
  std::printf("  top-10 match after reload: %zu/10\n", matches);

  DVamana::destroy(reloaded);
  std::remove(path.c_str());
  if (matches == 10) {
    pass("svs/test_06_save_load");
    return 0;
  }
  fail("svs/test_06_save_load", "top-10 mismatch: " + std::to_string(matches));
  return 1;
}
