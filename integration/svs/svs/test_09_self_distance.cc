...
static const MetricType kMetrics[] = {MetricType::L2,
...
int main() {
  ...
  std::printf(
      "Measures get_distance(label, same_vector) for all "
      "compression/metric/dim combos\n\n");
    ...
    for (size_t di = 0; di < 4; ++di) {
      ...
      for (const auto& cfg : kConfigs) {
        ...
        if (cfg.is_leanvec) {
          ...
          st = DynamicVamanaIndexLeanVec::build(&idx, dim, metric, cfg.storage,
                                                td, build, search);
          ...
          st = svs_::build_svs(&idx, dim, metric, cfg.storage, 32, 128, 50,
                               alpha);
          if (!st.ok()) {
            res.skipped = true;
        ...
        if (metric == MetricType::INNER_PRODUCT &&
            res.stats.max_dist > global_max_ip)
  ...
  for (const auto& r : results) {
    if (r.skipped) {
      std::printf("%-12s %-4s %4zu | SKIPPED (%s)\n", r.compression, r.metric,
                  r.dim, r.skip_reason);
    } else {
      std::printf("%-12s %-4s %4zu | %12.8f %12.8f %12.8f %12.8f\n",
                  r.compression, r.metric, r.dim, r.stats.min_dist,
                  r.stats.max_dist, r.stats.mean_dist, r.stats.stddev);
    }
  }
  ...
  std::printf("Distance between vectors differing by 2.0 in one component:\n");
  ...
  for (const auto& cfg : kConfigs) {
    if (cfg.is_leanvec)
      continue;  // skip LeanVec for this check to keep it fast
    ...
    for (size_t mi = 0; mi < 2; ++mi) {
      ...
      DVamana* idx = nullptr;
      auto st =
          svs_::build_svs(&idx, dim, metric, cfg.storage, 32, 128, 50, alpha);
      if (!st.ok()) continue;

      auto vec1 = random_vec(dim);
      auto vec2 = vec1;  // copy
      vec2[0] +=
          2.0f;  // perturbation large enough to exceed epsilon at dim=128
      ...
      std::printf("%-12s %-4s %4zu | %12.8f\n", cfg.name, metric_name, dim,
                  dist);

      // A clearly different vector must not fall within the match window.
      float fp_threshold = kDefaultEpsilonPerDim * static_cast<float>(dim);
      if (dist <= fp_threshold) {
        fail("false-positive-check",
             std::string(cfg.name) + "/" + metric_name + "/dim=" +
                 std::to_string(dim) + ": dist=" + std::to_string(dist) +
                 " <= epsilon=" + std::to_string(fp_threshold));
        DVamana::destroy(idx);
        return 1;
      }
      DVamana::destroy(idx);
    ...
    if (std::string(r.metric) == "L2") {
      if (r.stats.max_dist > threshold) {
        std::printf("  [EXCEED] %s/%s/dim=%zu: max=%.8f > threshold=%.8f\n",
                    r.compression, r.metric, r.dim, r.stats.max_dist,
                    threshold);
        ++threshold_failures;
      }
    }
  ...
  std::printf(
      "Global max self-distance (IP):  %g (raw dot product, NOT error)\n",
      global_max_ip);
  ...
  if (threshold_failures > 0) {
    fail("svs/test_09_self_distance",
         std::to_string(threshold_failures) +
             " L2 self-distance(s) exceeded kDefaultDistanceMatchEpsilonPerDim."
             " Consider raising the constant in vector_svs.h.");
    return 1;
  }
