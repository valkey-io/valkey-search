"""
Save/restore tests for BFLOAT16 vector indexes.

Mirror of test_fp16_saverestore.py for the BFLOAT16 data type. BF16 has the
same byte width as FP16 but a different bit layout (1 sign + 8 exp + 7
mantissa, equivalent to the top 16 bits of FP32). The C++ side disambiguates
the two formats via the VectorDataType enum, not by element size, so a
silent FP16/BF16 misroute would surface here as KNN result divergence
across the save/restore round trip.
"""
...
from indexes import Index, Vector, Numeric, Tag, bfloat16_to_bytes
...
def bf16_hnsw_index(name: str = "bf16_hnsw_index") -> Index:
    return Index(
        name,
        [
            Vector(
...
def bf16_hnsw_vector_only_index(name: str = "bf16_hnsw_vector_only") -> Index:
    return Index(
        name,
        [Vector("v", DIM, type="HNSW", m=4, efc=40, data_type="BFLOAT16")],
    )


def bf16_flat_index(name: str = "bf16_flat_index") -> Index:
    return Index(
        name,
        [Vector("v", DIM, type="FLAT", data_type="BFLOAT16"), Numeric("n"), Tag("t")],
    )


# (factory, num_vectors) pairs used by the parametrized matrix tests.
INDEX_VARIANTS = [
    (bf16_hnsw_index, NUM_VECTORS_HNSW),
    (bf16_hnsw_vector_only_index, NUM_VECTORS_HNSW),
    (bf16_flat_index, NUM_VECTORS_FLAT),
]
...
def query_vector_for_row(row: int) -> bytes:
    """Build the BF16-packed query vector that exactly matches the data
    written by Vector.make_value for the given row (column index = 0)."""
    return bfloat16_to_bytes([float(i + row) for i in range(DIM)])
...
def do_bf16_save_restore(test, index: Index, num_vectors: int):
    ...
    assert pre_results == post_results, (
        f"BF16 KNN results changed across save/restore for index {index.name}\n"
...
class TestBFloat16SaveRestore_v1_v1(ValkeySearchTestCaseDebugMode):
    ...
    def test_bf16_saverestore_v1_v1(self, index_factory, num_vectors):
        do_bf16_save_restore(self, index_factory(), num_vectors)


class TestBFloat16SaveRestore_v1_v2(ValkeySearchTestCaseDebugMode):
    def append_startup_args(self, args):
        args = super().append_startup_args(args)
        args["search.rdb_write_v2"] = "no"
    ...
    def test_bf16_saverestore_v1_v2(self, index_factory, num_vectors):
        do_bf16_save_restore(self, index_factory(), num_vectors)


class TestBFloat16SaveRestore_v2_v1(ValkeySearchTestCaseDebugMode):
    def append_startup_args(self, args):
        args = super().append_startup_args(args)
    ...
    def test_bf16_saverestore_v2_v1(self, index_factory, num_vectors):
        do_bf16_save_restore(self, index_factory(), num_vectors)


class TestBFloat16SaveRestore_v2_v2(ValkeySearchTestCaseDebugMode):
    def append_startup_args(self, args):
        args = super().append_startup_args(args)
        args["search.rdb_write_v2"] = "yes"
        args["search.rdb_read_v2"] = "yes"
    ...
    def test_bf16_saverestore_v2_v2(self, index_factory, num_vectors):
        do_bf16_save_restore(self, index_factory(), num_vectors)


# --------------------------------------------------------------------------
# Schema-only check: the index definition (BFLOAT16 + dim + algorithm) must
# survive a save/restore even when no documents have been written.
# --------------------------------------------------------------------------

class TestBFloat16SaveRestoreSchemaOnly(ValkeySearchTestCaseDebugMode):
    def append_startup_args(self, args):
        args = super().append_startup_args(args)
        args["search.rdb_write_v2"] = "yes"
        args["search.rdb_read_v2"] = "yes"
    ...
    def test_bf16_hnsw_schema_survives_restart_with_no_data(self):
        index = bf16_hnsw_index("bf16_hnsw_empty")
        ...
        # Index must come back with BF16 still in effect: writing FLOAT32
        # bytes (4*DIM) into a BFLOAT16 (2*DIM) field is a hard rejection
        # downstream, so a successful BF16 round-trip means we can write
        # BF16-sized payloads after restart.
