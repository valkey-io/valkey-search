...
class TestExpired(ValkeySearchTestCaseBase):
    def test_expired_hashes_should_remove_from_index(self):
        ...
        hnsw_index = Index(index_name, [Vector("v", 3, type="HNSW", m=2, efc=1), Numeric("n")])

        num_of_docs = 500
        hnsw_index.create(client)
        hnsw_index.load_data(client, num_of_docs)
        ft_info = FTInfoParser(client.execute_command("FT.INFO", index_name))
        ...
        waiters.wait_for_equal(lambda: FTInfoParser(client.execute_command("FT.INFO", index_name)).num_docs, num_of_docs / 2, timeout=5)