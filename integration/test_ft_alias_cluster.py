...
class TestFTAliasNameCollisionCluster(ValkeySearchClusterTestCase):
    ...
    def test_aliasadd_name_same_as_existing_index_cluster(self):
        """FT.ALIASADD where alias name matches a real index is accepted in cluster."""
        node0 = self.new_client_for_primary(0)
        assert node0.execute_command(*CREATE_TAG_INDEX) == b"OK"
        assert node0.execute_command(*CREATE_TAG_INDEX_2) == b"OK"
        self._wait_for_index_on_all_nodes(INDEX_NAME)
        ...
        # ALIASADD succeeds even though INDEX_NAME is a real index.
        assert node0.execute_command(
            "FT.ALIASADD", INDEX_NAME, INDEX_NAME_2
        ) == b"OK"
        # FT.INFO resolves to the real index (direct lookup wins).
        info = list(node0.execute_command("FT.INFO", INDEX_NAME))
...
class TestFTAliasConcurrentOperationsCluster(ValkeySearchClusterTestCase):
    ...
    def test_concurrent_aliasadd_different_aliases(self):
        """Multiple nodes adding different aliases concurrently all succeed."""
        node0 = self.new_client_for_primary(0)
        assert node0.execute_command(*CREATE_TAG_INDEX) == b"OK"
        self._wait_for_index_on_all_nodes(INDEX_NAME)
        ...
        def add_alias(node_idx):
            try:
                ...
                results[node_idx] = client.execute_command(
                    "FT.ALIASADD", f"alias_from_node_{node_idx}", INDEX_NAME
                )
        ...
        # All should succeed — different alias names don't conflict.
        for i in range(self.CLUSTER_SIZE):
            assert errors[i] is None, f"Node {i} got error: {errors[i]}"
            assert results[i] == b"OK"
