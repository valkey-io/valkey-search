...
- **SVS** consults the predicate *post-traversal, per batch* (`bindings/c/src/filtered_search.hpp:189-218`). The Vamana `greedy_search` receives no predicate; it returns a fixed-size batch that the filter is then applied to, and the batch iterator is advanced until `k` filter-passing candidates are collected or the `filter_rate` hint's confidence budget is exhausted -- at which point SVS returns an empty result set (`svs_c.h:444-446`), where HNSW would return a partial result.
