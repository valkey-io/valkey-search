...
FilterParser::ParseQueryAttributes(bool expect_arrow_after) {
      ...
      if (!IsEnd() && Peek() == '@') {
        ...
        if (Match('[')) {
          ...
          if (MatchInsensitive("VECTOR_RANGE")) {
            ...
            // Check for suffix query attributes: =>{$yield_distance_as: ...}
            // This is the Redis-compatible syntax where attributes follow the
            // closing ']'.
            if (!query_attrs.has_value() && !IsEnd()) {
              auto saved_suffix_pos = pos_;
              SkipWhitespace();
              if (!IsEnd() && pos_ + 1 < expression_.length() &&
                  expression_[pos_] == '=' && expression_[pos_ + 1] == '>') {
                pos_ += 2;  // consume '=>'
                SkipWhitespace();
                if (!IsEnd() && Peek() == '{') {
                  ++pos_;  // consume '{'
                  auto suffix_attrs_or = ParseQueryAttributes(false);
                  if (suffix_attrs_or.ok()) {
                    query_attrs = std::move(*suffix_attrs_or);
                  } else {
                    // Not a valid suffix attributes block — restore position
                    pos_ = saved_suffix_pos;
                  }
                } else {
                  pos_ = saved_suffix_pos;
                }
            ...
            // Attach query attributes if present (from prefix or suffix)
