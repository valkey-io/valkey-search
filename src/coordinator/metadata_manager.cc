...
void MetadataManager::BroadcastMetadata(
  ...
  for (const auto &node : primary_nodes) {
    int result = ValkeyModule_SendClusterMessage(
        ctx, node.node_id.c_str(), kMetadataBroadcastClusterMessageReceiverId,
        payload.c_str(), payload.size());
    if (result != VALKEYMODULE_OK) {
      VMSDK_LOG(WARNING, ctx) << "Failed to send cluster message to node "
                              << node.node_id << ", result: " << result;
    }
  }
}
...
std::vector<vmsdk::cluster_map::NodeInfo> MetadataManager::GetPrimaryNodes(
  ...
  if (!cluster_map) {
    ...
    cluster_map = ValkeySearch::Instance().GetOrRefreshClusterMap(safe_context);
    if (!vmsdk::IsMainThread()) {
      ValkeyModule_ThreadSafeContextUnlock(thread_safe_ctx.get());
    }
  }
