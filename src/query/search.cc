...
absl::Status query::SearchParameters::PreParseQueryString() {
  ...
  while (search_pos < filter_expression.size()) {
    ...
    // Check if this '=>' is part of a query attributes block.
    // Two patterns to detect:
    //   1. Prefix: {$yield_distance_as: ...}=>@field:[VECTOR_RANGE ...]
    //   2. Suffix: @field:[VECTOR_RANGE ...]=>{ $yield_distance_as: ...}
    // We must not confuse a tag value like {2}=> with query attributes.
    // Query attributes always involve '{$'.
    bool is_query_attr_delimiter = false;
    if (found > 0) {
      // Check prefix pattern: look backwards past whitespace for '}'.
    ...
    // Check suffix pattern: look forward past '=>' and optional whitespace
    // for '{$'. This handles ]=>{$yield_distance_as: ...} syntax.
    if (!is_query_attr_delimiter) {
      ...
      if (after_arrow + 1 < filter_expression.size() &&
          filter_expression[after_arrow] == '{' &&
          filter_expression[after_arrow + 1] == '$') {
        is_query_attr_delimiter = true;
      }
    }
    if (!is_query_attr_delimiter) {
