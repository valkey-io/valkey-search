...
std::optional<float> ScoreNode(const Predicate *predicate,
  ...
  switch (predicate->GetType()) {
    ...
    case PredicateType::kText: {
      ...
      // co-located in the posting entry (identical across postings for one key),
      // so the same LookupKey that yields tf yields it — no separate per-key
      // scoring-map probe. avg_doc_len is corpus-wide (precomputed in
