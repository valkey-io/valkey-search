...
float ComputeMatchedPredicateScore(const Predicate *predicate) {
  ...
  switch (predicate->GetType()) {
    ...
    case PredicateType::kNegate: {
      // Negation matched, contribute its weight
      return 1.0f * predicate->GetWeight();
    }
...
absl::StatusOr<std::vector<indexes::Neighbor>> SearchNonVectorQuery(
  ...
  auto results_appender =
      [&neighbors, &parameters, max_keys, &fetch_limited, weighted_score](
          const InternedStringPtr &key,
          absl::flat_hash_set<const char *> &top_keys) -> bool {
    ...
    neighbors.emplace_back(indexes::Neighbor{key, weighted_score});
    return true;
  };
