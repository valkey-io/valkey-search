...
class FlatPositionMapTest : public ::testing::Test {
 protected:
  ...
  void VerifyIteration(const FlatPositionMap& flat_map,
...
class RandomPositionMapGenerator {
 public:
  ...
  // Focuses on partition boundaries (128 bytes ~= 40-50 positions with small deltas)
  ...
  uint64_t GenerateFieldMask(size_t num_fields) {
    ...
    if (category < 2) {  // 20% all bits set
      ...
      size_t bit = std::uniform_int_distribution<size_t>(0, num_fields - 1)(rng_);
      return 1ULL << bit;
    } else {  // 60% random multiple bits
      uint64_t mask = 0;
      size_t num_bits = std::uniform_int_distribution<size_t>(1, num_fields)(rng_);
      for (size_t i = 0; i < num_bits; ++i) {
        size_t bit = std::uniform_int_distribution<size_t>(0, std::min(num_fields, size_t{64}) - 1)(rng_);
        mask |= (1ULL << bit);
      }
  ...
  std::vector<Position> GenerateSkipTargets(
    ...
    for (size_t i = 0; i < num_targets; ++i) {
      ...
      if (category < 5 && !positions.empty()) {  // 50% exact position
        size_t idx = std::uniform_int_distribution<size_t>(0, positions.size() - 1)(rng_);
        targets.push_back(positions[idx].first);
      } else if (category < 8) {  // 30% random position in range (>= min_pos)
        targets.push_back(std::uniform_int_distribution<Position>(min_pos, max_pos)(rng_));
      } else {  // 20% beyond end
        targets.push_back(max_pos + std::uniform_int_distribution<Position>(1, 1000)(rng_));
      }
    }
    ...
    for (const auto& target : skip_targets) {
      ...
      if (iter.IsValid()) {
        EXPECT_GE(iter.GetPosition(), target)
            << "Test " << test_num << " failed: skip forward didn't reach target";
        
        // If exact match claimed, verify it
        if (exact_match) {
          EXPECT_EQ(iter.GetPosition(), target)
              << "Test " << test_num << " failed: exact match claimed but position differs";
        }
        ...
        if (iter.IsValid()) {
          EXPECT_GT(iter.GetPosition(), prev_pos)
              << "Test " << test_num << " failed: positions not monotonic after skip";
        }
      }
    }
  }
...
TEST_F(FlatPositionMapTest, RandomSkipForwardPatterns) {
  ...
  for (int test = 0; test < 100; ++test) {
    ...
    for (size_t i = 0; i < num_skips; ++i) {
      ...
      size_t target_idx = std::uniform_int_distribution<size_t>(0, num_elements - 1)(rng);
      ...
      ASSERT_TRUE(iter.IsValid()) << "Test " << test << " skip " << i << " failed";
      EXPECT_GE(iter.GetPosition(), target);
...
TEST_F(FlatPositionMapTest, VeryLargeMapScenarios) {
  // Test very large position maps (100K-500K positions) across multiple scenarios
  ...
  for (size_t target_size : test_sizes) {
    ...
    if (target_size == 100000) {
      ...
      for (size_t skip_test = 0; skip_test < 10; ++skip_test) {
        size_t target_idx = std::uniform_int_distribution<size_t>(0, target_size - 1)(rng);
        PositionIterator iter(flat_map);
