...
class StabilityTests(parameterized.TestCase):
    ...
    @parameterized.named_parameters(
        ...
        # ),
        dict(
            testcase_name="hnsw_memory_limit_500mb",
            config=stability_runner.StabilityTestConfig(
                index_name="hnsw_memory_test",
                ports=(7087, 7088, 7089),
                index_type="HNSW",
                vector_dimensions=128,
                bgsave_interval_sec=15,  # Normal interval
                ftcreate_interval_sec=10,  # Normal interval
                ftdropindex_interval_sec=10,  # Normal interval
                flushdb_interval_sec=0,  # Disabled to maximize memory usage
                randomize_bg_job_intervals=True,
                num_memtier_threads=20,  # Increased for more HSET throughput
                num_memtier_clients=20,  # Increased for more HSET throughput
                num_search_clients=10,
                insertion_mode="time_interval",
                test_time_sec=180,  # 3 minutes to fill memory
                test_timeout=240,
                keyspace_size=5000000,  # Large keyspace to fill 500MB
                use_coordinator=False,
                replica_count=0,
                repl_diskless_load="swapdb",
                maxmemory="200mb",
                maxmemory_policy="allkeys-lru",  # LRU eviction when limit hit
            ),
        ),
        # dict(
