...
class PostingTest : public ValkeySearchTest {
 protected:
  void SetUp() override {
    ...
    boolean_schema_ = MockIndexSchema::Create(
                          &fake_ctx_, "boolean_schema", key_prefixes,
                          std::make_unique<MockAttributeDataType>(), nullptr)
                          .value();

    positional_schema_ = MockIndexSchema::Create(
                             &fake_ctx_, "positional_schema", key_prefixes,
                             std::make_unique<MockAttributeDataType>(), nullptr)
                             .value();

    // Create postings with different configurations for testing using
    // IndexSchema
    boolean_postings_ = new Postings(false, 3);  // Boolean search, 3 fields
    positional_postings_ =
        new Postings(true, 5);  // Positional search, 5 fields
  }
...
TEST_F(PostingTest, BooleanSearchInsertPosting) {
  ...
  boolean_postings_->InsertPosting(InternKey("doc1"),
                                   0);  // field 0, position ignored
  boolean_postings_->InsertPosting(InternKey("doc1"), 1,
                                   100);  // field 1, position 100 ignored
  boolean_postings_->InsertPosting(InternKey("doc2"),
                                   2);  // field 2, position ignored
...
TEST_F(PostingTest, PositionalSearchInsertPosting) {
  ...
  positional_postings_->InsertPosting(InternKey("doc1"), 0,
                                      10);  // field 0, position 10
  positional_postings_->InsertPosting(InternKey("doc1"), 1,
                                      20);  // field 1, position 20
  positional_postings_->InsertPosting(
      InternKey("doc1"), 2,
...
TEST_F(PostingTest, InsertPostingDefaultPosition) {
  ...
  boolean_postings_->InsertPosting(
      InternKey("doc1"), 0);  // Default position ignored in boolean mode
  boolean_postings_->InsertPosting(
...
TEST_F(PostingTest, SingleFieldOptimization) {
  // Test posting with single field (uses SingleFieldMask optimization
  // internally)
  std::vector<absl::string_view> single_key_prefixes = {"single:"};
  auto single_field_schema =
      MockIndexSchema::Create(
          &fake_ctx_, "single_field_schema", single_key_prefixes,
          std::make_unique<MockAttributeDataType>(), nullptr)
          .value();
...
TEST_F(PostingTest, BooleanVsPositionalBehavior) {
  ...
  boolean_postings_->InsertPosting(InternKey("doc1"), 0,
                                   100);  // position 100 ignored
  boolean_postings_->InsertPosting(InternKey("doc1"), 1,
                                   200);  // position 200 ignored
...
TEST_F(PostingTest, IteratorWithMultipleFields) {
  ...
  positional_postings_->InsertPosting(InternKey("doc1"), 0,
                                      10);  // Field 0 at position 10
  positional_postings_->InsertPosting(InternKey("doc1"), 2,
                                      10);  // Field 2 at position 10
  positional_postings_->InsertPosting(InternKey("doc1"), 1,
                                      20);  // Field 1 at position 20
