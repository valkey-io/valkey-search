...
std::string VectorAtScale(float scale) {
  ...
  return std::string(reinterpret_cast<const char *>(v), kVectorBytes);
}
...
class VectorRegistryStateMachineTest
 ...
 protected:
  ...
  std::string VectorIdentifier() const {
    return GetParam().json ? "$.vec" : "vec_id";
  }
  std::string NumericIdentifier() const {
    return GetParam().json ? "$.num" : "num_id";
  }
  std::string TagIdentifier() const {
  ...
  std::optional<std::string> VectorValue(FieldState state, float scale) const {
    switch (state) {
  ...
  std::optional<std::string> NumericValue(FieldState state) const {
    switch (state) {
      case FieldState::kAbsent:
        return std::nullopt;
      case FieldState::kValid:
  ...
  std::optional<std::string> TagValue(FieldState state) const {
    switch (state) {
      case FieldState::kAbsent:
        return std::nullopt;
      case FieldState::kValid:
  ...
  void WriteKey(absl::string_view key, FieldState vector_state,
    ...
    if (GetParam().mixed_schema) {
      set(NumericIdentifier(), NumericValue(numeric_state));
      set(TagIdentifier(), TagValue(tag_state));
    }
    Notify(key);
  }
  ...
  bool RegistrationExpected() const { return !GetParam().json; }

  size_t ExpectedEntries(size_t hash_entries) const {
...
// point of the registry) and the engine must not be re-shared.
TEST_P(VectorRegistryStateMachineTest, OverwriteValidWithIdenticalValid) {
  ...
  EXPECT_EQ(EntryCount(), ExpectedEntries(1));
  EXPECT_EQ(SharingHits(), hits_before) << "re-sharing an unchanged record";
}
...
TEST_P(VectorRegistryStateMachineTest, InvalidNonVectorFieldWithValidVector) {
  ...
  WriteKey(kKey, FieldState::kValid, 1.0f, FieldState::kInvalid,
           FieldState::kValid);
...
// --- untrack from a mutation worker thread -------------------------------

// VectorBase::RemoveRecordDueToError runs on a mutation worker thread and
// calls VectorRegistry::UntrackIfUnused, which -- when sharing is enabled --
// calls DetachFromValkeyHash, which calls ValkeyModule_OpenKey through
// vmsdk::MakeUniqueValkeyOpenKey. That helper asserts VerifyMainThread().
//
// A FLAT index frees its reference to the VectorRecord synchronously inside
// RemoveRecordImpl (unlike HNSW, whose soft delete keeps the element alive and
// therefore keeps use_count above 1), so overwriting a valid vector with an
// invalid one on a FLAT index drops the registry to the last reference on a
// worker thread and takes that path.
TEST_P(VectorRegistryStateMachineTest, UntrackFromWorkerThreadOnFlatIndex) {
  if (!SharingExpected()) {
    GTEST_SKIP() << "sharing only applies to HASH keys with sharing enabled";
  }
  GTEST_FLAG_SET(death_test_style, "threadsafe");
  VMSDK_EXPECT_DEATH(
      {
        // Rebuild the schema around a FLAT index driven by a real worker pool.
        index_schema_.reset();
        vector_index_.reset();
        numeric_index_.reset();
        tag_index_.reset();

        vmsdk::ThreadPool pool("writer-thread-pool-", 1);
        pool.StartWorkers();
        std::vector<absl::string_view> key_prefixes = {kKeyPrefix};
        auto schema_or = MockIndexSchema::Create(
            &fake_ctx_, "flat_schema", key_prefixes,
            std::make_unique<HashAttributeDataType>(), &pool);
        CHECK(schema_or.ok());
        index_schema_ = schema_or.value();
        auto flat = indexes::VectorFlat<float>::Create(
            CreateFlatVectorIndexProto(kDims, data_model::DISTANCE_METRIC_L2,
                                       /*initial_cap=*/64, /*block_size=*/16),
            VectorIdentifier(),
            data_model::AttributeDataType::ATTRIBUTE_DATA_TYPE_HASH,
            /*db_num=*/0);
        CHECK(flat.ok());
        vector_index_ = *flat;
        CHECK(index_schema_
                  ->AddIndex(kVectorAlias, VectorIdentifier(), vector_index_)
                  .ok());

        WriteKey(kKey, FieldState::kValid, 1.0f);
        WaitWorkerTasksAreCompleted(pool);
        CHECK(Lookup(kKey).first != nullptr);

        // Overwrite with an unusable vector: the worker thread reaches
        // UntrackIfUnused -> DetachFromValkeyHash -> ValkeyModule_OpenKey.
        WriteKey(kKey, FieldState::kInvalid);
        WaitWorkerTasksAreCompleted(pool);
        pool.JoinWorkers();
      },
      "Check failed: IsMainThread");
}
