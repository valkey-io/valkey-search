...
class BruteforceSearch : public AlgorithmInterface<dist_t> {
 public:
  ...
  std::priority_queue<std::pair<dist_t, labeltype>> searchKnn(
    ...
    for (int i = 0; i < k; i++) {
      dist_t dist = fstdistfunc_(query_data, *(char **)(*data_)[i],
    ...
    for (int i = k; i < cur_element_count_ &&
  ...
  absl::Status SaveIndex(OutputStream &output,
    ...
    for (int i = 0; i < cur_element_count_; i++) {
      memcpy(buf.data(),
             vector_codec->Encode(*(char **)(*data_)[i], vector_size_, false)
                 .data(),
             vector_size_);
  ...
  absl::Status LoadIndex(InputStream &input, SpaceInterface<dist_t> *s,
    ...
    for (int i = 0; i < cur_element_count_; i++) {
      ...
      *(char **)(*data_)[i] =
          vector_codec->Decode(id, chunk->data(), vector_size_);
