    ...
    default:
      ...
      std::vector<NodeInfo> ClusterMap::GetRandomTargets() const {
        ...
        for (const auto& [shard_id, shard_info] : shards_) {
          ...
          if (prefer_local && shard_info.replicas.size() > 0) {
            // When preferring local, try to find a local node first
            if (shard_info.primary.has_value() &&
                shard_info.primary->is_local) {
              random_targets.push_back(shard_info.primary.value());
              found_local = true;
            } else {
              for (const auto& replica : shard_info.replicas) {
                if (replica.is_local) {
                  random_targets.push_back(replica);
                  found_local = true;
                  break;
                }
              }
      ...
      std::optional<NodeInfo> ClusterMap::ParseNodeInfo(
          ValkeyModuleCallReply * node_arr, const char* my_node_id,
          bool is_primary) {
        ...
        const char* node_primary_endpoint_char =
            ValkeyModule_CallReplyStringPtr(primary_endpoint_reply,
        ...
        auto insert_metadata = [&additional_network_metadata](
          ...
          size_t key_len;
          const char* key_str =
              ValkeyModule_CallReplyStringPtr(key_reply, &key_len);
          size_t val_len;
          const char* val_str =
              ValkeyModule_CallReplyStringPtr(val_reply, &val_len);
        ...
        switch (ValkeyModule_CallReplyType(reply_metadata)) {
          case VALKEYMODULE_REPLY_MAP: {
            ...
            int idx = 0;
            while (ValkeyModule_CallReplyMapElement(reply_metadata, idx,
                                                    &map_key, &map_val) ==
                   VALKEYMODULE_OK) {
          ...
          case VALKEYMODULE_REPLY_ARRAY: {
            ...
            for (size_t i = 0; i + 1 < array_len; i += 2) {
              auto arr_key =
                  ValkeyModule_CallReplyArrayElement(reply_metadata, i);
              auto arr_val =
        ...
        SocketAddress addr{
            .primary_endpoint = std::string(node_primary_endpoint_char,
                                            node_primary_endpoint_len),
      ...
      bool ClusterMap::ProcessSlotRange(
        ...
        CHECK(ValkeyModule_CallReplyType(slot_range) ==
        ...
        auto primary_node_opt =
            ParseNodeInfo(primary_node_arr, my_node_id, true);
        ...
        for (size_t j = 3; j < slot_len; j++) {
          ...
          if (!replica_opt.has_value()) {
            VMSDK_LOG(WARNING, nullptr)
                << "Skipping invalid replica in slot range [" << start << "-"
                << end << "]";
            is_consistent_ = false;
            return false;
          }
        ...
        if (is_new_shard) {
          ...
          if (!IsExistingShardConsistent(shard_it->second, primary_node,
                                         replicas)) {
            VMSDK_LOG(WARNING, nullptr)
        ...
        slot_ranges.push_back({static_cast<uint16_t>(start),
                               static_cast<uint16_t>(end), shard_id});
      ...
      bool ClusterMap::CheckClusterMapFull() {
        if (slot_to_shard_map_.empty() ||
            slot_to_shard_map_.begin()->first != 0) {
          return false;
        }
        ...
        for (const auto& [start_slot, range_and_shard] : slot_to_shard_map_) {
          CHECK(start_slot >= expected_next)
              << "Slot " << start_slot
              << " overlaps with previous range ending at "
              << (expected_next - 1);
      ...
      std::shared_ptr<ClusterMap> ClusterMap::CreateNewClusterMap(
        ...
        CHECK(ValkeyModule_CallReplyType(reply.get()) ==
              VALKEYMODULE_REPLY_ARRAY)
            << kValkeyModuleCallErrorMsg;
        ...
        // fix shard pointers after finished inserting all shards into shards_
        // map inserting additional shards into shards_ map after this would
        // cause rebalance of btree and invalidate pointers in shards_ map
        ...
        new_map->cluster_slots_fingerprint_ =
            new_map->ComputeClusterFingerprint();
      ...
      void ClusterMap::PrintClusterMap(std::shared_ptr<ClusterMap> map) {
        ...
        VMSDK_LOG(NOTICE, nullptr) << "cluster_slots_fingerprint_: "
        ...
        if (owned_count > 0) {
          ...
          if (!owned_ranges.empty()) {
            VMSDK_LOG(NOTICE, nullptr)
                << "owned_slots_ ranges: " << owned_ranges;
          }
        }
        ...
        for (const auto& [shard_id, shard_info] : map->shards_) {
          ...
          for (size_t i = 0; i < shard_info.replicas.size(); ++i) {
            ...
            VMSDK_LOG(NOTICE, nullptr)
                << "    port: " << replica.socket_address.port;
            if (!replica.additional_network_metadata.empty()) {
              VMSDK_LOG(NOTICE, nullptr) << "    additional_network_metadata:";
              for (const auto& [key, value] :
                   replica.additional_network_metadata) {
                VMSDK_LOG(NOTICE, nullptr) << "      " << key << ": " << value;
              }
            } else {
              VMSDK_LOG(NOTICE, nullptr)
      ...
      bool IsSystemUnderLowUtilization() {
        ...
        VMSDK_LOG(WARNING, nullptr)
            << "IsSystemUnderLowUtilization queue_wait_time:" << queue_wait_time
            << " threshold:" << threshold;
