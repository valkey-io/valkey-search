...
void ThreadMonitor::Start(pthread_t thread_id) {
...
absl::StatusOr<double> ThreadMonitor::GetThreadCPUPercentage() {
  ...
  int64_t cpu_time_elapsed_us = current_cpu_time - last_cpu_time_;
  if (current_wall_time_micro < last_wall_time_micro_) {
    return absl::InternalError("Internal error in CPU calculation");
  }
  int64_t wall_time_elapsed = current_wall_time_micro - last_wall_time_micro_;
...
absl::StatusOr<uint64_t> ThreadMonitor::GetCPUTime() const {
  ...
  thread_inspect_t target = ConvertToMachThread(tid);

  if (thread_info(target, THREAD_BASIC_INFO, (thread_info_t)&info, &count) !=
      KERN_SUCCESS) {
    return absl::InternalError(
        absl::StrFormat("Failed to get thread info for thread %d",
  ...
  if (pthread_getcpuclockid(thread_id_, &cid) != 0) {
    return absl::InternalError(
        absl::StrFormat("Failed to get CPU clock ID for thread %d",
                        static_cast<unsigned int>(thread_id_)));
  }

  if (clock_gettime(cid, &ts) != 0) {
    return absl::InternalError(
        absl::StrFormat("Failed to get value of clock for thread %d",
                        static_cast<unsigned int>(thread_id_)));
  }
