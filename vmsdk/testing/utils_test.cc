...
TEST_F(UtilsTest, DestructByMainThread) {
  ...
  EXPECT_TRUE(thread_pool.Schedule(
      [&]() {
        ...
        // The destruction is enqueued here via ptr.reset() going out of scope
        // or being explicitly called.
        ptr.reset();
      },
      ThreadPool::Priority::kLow));
  ...
  // The object hasn't been deleted yet because the callback is queued.
  EXPECT_FALSE(deleted);
  // Run pending callbacks on the main thread
